...
export default function FanWallGrid({
  ...
  // ✅ Per-wall tab state
  const [wallTabs, setWallTabs] = useState<Record<string, WallTab>>({});
  ...
  /* ------------------------------------------------------ */
  /*  EXPORT (Guestbook HTML → Print to PDF)                */
  /* ------------------------------------------------------ */
  function handleExportGuestbook(wallId: string) {
    const url = `${window.location.origin}/api/export/fanwall/guestbook?wallId=${wallId}`;
  ...
  function getTab(wallId: string): WallTab {
    return wallTabs[wallId] || "controls";
  }

  function setTab(wallId: string, tab: WallTab) {
    setWallTabs(prev => ({ ...prev, [wallId]: tab }));
  }
              ...
              >
                <div>
                  ...
                  <div className={cn("flex justify-center gap-2 mb-2")}>
                    <button
                      type="button"
                      onClick={() => setTab(wall.id, "controls")}
                      className={cn(
                  ...
                  {tab === "controls" ? (
                    ...
                    <div className={cn("flex flex-col gap-2 items-center")}>
                      <button
                        ...
                        className={cn(
                          "w-full px-3 py-2 rounded-md text-sm font-extrabold transition shadow-md",
                          "bg-white/90 text-black hover:bg-white"
                        )}
                      >
                        📄 Export Guestbook (Print / Save as PDF)
                      </button>

                      <p className={cn("text-xs text-white/70 leading-snug")}>
                        Opens a print-ready guestbook where <b>each approved post is one page</b>.
                        Use your browser’s <b>Print → Save as PDF</b>.
                      </p>
                    </div>
                  )}
                </div>
