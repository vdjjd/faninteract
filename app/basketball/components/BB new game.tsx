...
const GlobalAnimations = () => (
  <style>{`
    /* --------------- BALL SPIN (Arcade / NBA Jam) --------------- */
    @keyframes ballSpinFast {
      0% { transform: rotate(0deg); }
      100% { transform: rotate(720deg); } /* 2 full spins per second */
    }
    ...
    /* --------------- NET FLUTTER (Arcade 2-stage recoil) --------------- */
...
/* NEW ARCADE PHYSICS BALL STATE */
interface BallState {
  active: boolean;
  progress: number;      // 0 → 1 arc
  power: number;         // 0 → 1 shooter
  x: number;             // horizontal sway
  y: number;             // vertical lift + forward travel
  scale: number;         // depth (1 → 0.55)
  spin: "slow" | "medium" | "fast";
  rimShake: "soft" | "medium" | "hard" | null;
  netStage: 0 | 1 | 2;   // flutter stage
}
...
export default function ActiveBasketballPage({ gameId }: { gameId: string }) {
  ...
  useEffect(() => {
    ...
    if (timeLeft <= 0) return setTimerExpired(true);
  ...
  /* ============================================================
     PLAYER LOADING + REALTIME UPDATES
  ============================================================= */
      ...
      {Array.from({ length: 10 }).map((_, i) => {
        ...
        return (
          ...
          >
            ...
            {/* ============================================================
               RIM — NOW WITH POWER-BASED ARCADE SHAKE
            ============================================================= */}
            <div
              style={{
                position: "absolute",
            ...
            {/* ============================================================
               BALL — FULL ARCADE PHYSICS (X/Y/Z, SPIN, SHADOW)
            ============================================================= */}
            {ball.active ? (
              <>
                {/* BALL SHADOW */}
