...
export default function TriviaActiveWall({ trivia }: TriviaActiveWallProps) {
  ...
  /* -------------------------------------------------- */
  /* ✅ Auto-fit question text to ≤3 lines                */
  /* -------------------------------------------------- */
  useEffect(() => {
    ...
    const handle = window.setTimeout(() => {
      ...
      // If content is taller than 3 lines, step down font mode
      if (actual > maxHeight + 1 && questionFontMode < maxMode) {
        setQuestionFontMode(questionFontMode + 1);
        return;
      }

      // If content is comfortably under the 3-line block and we previously shrank it,
      // step back up (but not above mode 0)
